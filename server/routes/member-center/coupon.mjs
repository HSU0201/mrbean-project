...
router.get("/", authenticate, async (req, res) => {
  ...
  try {
    ...
    // 已過期
    const [resultsExpired, fieldExpired] = await connection.execute(
      "SELECT coupon.coupon_img, coupon.coupon_name, coupon_user_mapping.*, DATE_FORMAT(coupon_user_mapping.expire_at, '%Y-%m-%d') AS formatted_expire_at, DATE_FORMAT(coupon_user_mapping.used_at, '%Y-%m-%d') AS formatted_used_at FROM `coupon_user_mapping` JOIN coupon ON coupon.coupon_id = coupon_user_mapping.coupon_id WHERE expire_at < ? and valid = 1",
      [today]
    );
    // 已使用
    const [resultsUsed, fieldUsed] = await connection.execute(
      "SELECT coupon.coupon_img, coupon.coupon_name, coupon_user_mapping.*, DATE_FORMAT(coupon_user_mapping.expire_at, '%Y-%m-%d') AS formatted_expire_at, DATE_FORMAT(coupon_user_mapping.used_at, '%Y-%m-%d') AS formatted_used_at FROM `coupon_user_mapping` JOIN coupon ON coupon.coupon_id = coupon_user_mapping.coupon_id WHERE valid = 0"
    );
