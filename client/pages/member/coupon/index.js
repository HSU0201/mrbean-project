...
export default function MyCoupon() {
  ...
  const [couponExpiredList, setCouponExpiredList] = useState([])
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setCouponExpiredList(result.expired)
        setCouponUsedList(result.used)
      } catch (error) {
        console.error(error)
      }
    }
          ...
          <div className={styles['main-content-content']}>
            ...
            <div className="couponList row g-3 py-1">
              ...
              (couponListStatus === 0 || couponListStatus === 3)
                ...
                : ''}
              {couponExpiredList && couponListStatus === 4
                ? couponExpiredList.map((couponData, index) => {
                    return (
                      <CouponCard
                        key={index}
                        couponData={couponData}
                ...
                : ''}
              {couponUsedList && couponListStatus === 4
                ? couponUsedList.map((couponData, index) => {
                    return (
                      <CouponCard
                        key={index}
                        couponData={couponData}
