...
export default function Step2() {
  ...
  // useEffect(() => {
  //   if (driving === '超商取貨') {
  //     CVSlogistic.current.setAttribute('disabled', 'true')
  //   } else {
  //     CVSlogistic.current.removeAttribute('disabled')
  //   }
  // }, [driving])
  const {
    // checkChecked,
  ...
  useEffect(() => {
    if (window.location.search) {
      ...
      // setStorageInfo({
      //   ...storageInfo,
      //   userInfo: { ...storageInfo.userInfo, address: urlParams },
      // })
      setUserinfo((prevUserInfo) => ({
        ...prevUserInfo,
        address: urlParams,
                ...
                >
                  <div className={`col-1 ${styles['img-container']}`}>
                    <Image
                      ...
                      // className={styles["course-img-quick"]}
                      width={80}
                      height={80}
            ...
            <input
              ...
              onChange={(e) => {
                if (!isNaN(e.target.value)) {
                  ...
                  // if (e.target.value > userPoints && e.target.value < total) {
                  //   setusedPointValue(Number(userPoints))
                  // } else if (
                  //   e.target.value < userPoints &&
                  //   e.target.value > total
                  // ) {
                  //   setusedPointValue(Number(total))
                  // } else {
                  //   setusedPointValue(Number(e.target.value))
                  // }
                }
              }}
            />
