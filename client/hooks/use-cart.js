...
import users from '@/data/member.json'
...
export function CartProvider({ children }) {
  ...
  // const [checkChecked, setCheckChecked] = useState([]) //修改
  ...
  useEffect(() => {
    if (localStorage.getItem('cart')) {
      ...
      // setCheckChecked(JSON.parse(localStorage.getItem('checked'))) 修改
    }
  }, [])

  useEffect(() => {
    localStorage.setItem('cart', JSON.stringify(cart))
    // localStorage.setItem('checked', JSON.stringify(handleChecked(cart))) 修改
    // setCheckChecked(JSON.parse(localStorage.getItem('checked'))) 修改
  }, [cart])
  ...
  const removeItem = (cart, id, category, name) => {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        // MySwal.fire({
        //   title: `已成功移除${category == 'product' ? '商品' : '課程'}`,
        //   text: `已將 "${name}" 移出購物車`,
        //   icon: 'success',
        //   confirmButtonText: '確定',
        ...
        // Toast.fire({
        //   icon: 'success',
        //   title: `已將 ${category == 'product' ? '商品' : '課程'} 移出購物車`,
        // })
        toast.success(
